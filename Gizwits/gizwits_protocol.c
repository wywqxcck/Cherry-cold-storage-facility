...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(uint8_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagco2control)
    {
        info->event[info->num] = EVENT_co2control;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valueco2control,(uint8_t *)&issuedData[byteFieldOffset],co2control_LEN);
        byteFieldOffset += co2control_LEN;
        dataPoints->valueco2control = gizX2Y(co2control_RATIO,  co2control_ADDITION, exchangeBytes(elongateP0FormTmp.devStatus.valueco2control));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueco2control != cur->valueco2control)
    {
        GIZWITS_LOG("valueco2control Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagco2control = 1;
        ret = 1;
    }
    ...
    if(last->valueweight != cur->valueweight)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueweight Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagweight = 1;
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , uint8_t *outData,uint32_t *outDataLen)
{
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagco2control)
    {
        devStatusTmp.valueco2control = exchangeBytes(gizY2X(co2control_RATIO,  co2control_ADDITION, dataPoints->valueco2control)); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueco2control,co2control_LEN);
        byteFieldOffset += co2control_LEN;
    }
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagweight)
    {
        devStatusTmp.valueweight = exchangeBytes(gizY2X(weight_RATIO,  weight_ADDITION, dataPoints->valueweight)); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueweight,weight_LEN);
        byteFieldOffset += weight_LEN;
    }
